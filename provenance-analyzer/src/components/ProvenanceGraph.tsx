...
const ProvenanceGraph = ({ provenanceGraph }: IChartGenerator) => {
  ...
  xScale.domain([new Date(provenanceGraph.startTime), new Date(provenanceGraph.stopTime)]);
  console.log(xScale);
  const barHeight = 50;
  const yOffset = 50;
  const bars = [{
      x: 0,
      y: yOffset,
      width: xScale(provenanceGraph.stopTime) - xScale(provenanceGraph.startTime),
      height: barHeight,
      fill: 'gray',
    }];
  ...
  provenanceGraph.nodes.forEach(node=>{
    if(node.event==='Start Task' || node.event === 'Finished Task'){
      return;
    }
    provNodes.push({
      x: xScale(node.time),
