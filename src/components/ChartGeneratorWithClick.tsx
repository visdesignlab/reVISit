...
interface IChartGeneratorWithClick {
  data: number[];
  selectedDataIndicies: number[];
  handleClick: Function;
}

const ChartGeneratorWithClick = ({
  data,
  selectedDataIndicies,
  handleClick,
}: IChartGeneratorWithClick) => {
  ...
  const bars = data.map((d, i) => {
    const y1 = yScale(d);
    const y2 = yScale(0);
    const isDataIndexSelected = selectedDataIndicies.includes(i);
    return {
      x: xScale(i),
      y: y1,
      height: y2 - y1,
      fill: isDataIndexSelected ? "pink" : colors(colorScale(d)),
    };
  });
        ...
        {bars.map((d, i) => {
          return (
            <rect
              onClick={(event) => {
                console.log(
                  "if you needed access to the event for your handler, here it is!",
                  event
                );
...
export default ChartGeneratorWithClick;
