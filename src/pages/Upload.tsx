               ...
               let filteredMetrics = condition.stats //metrics.filter(m => m.group.taskID == task.taskID && m.group.condition == condition);
                    ...
                    let background = nodes.map((n) => {
                      return <>
                        <rect className='count' key={'background_' + n.event} style={{ fill: colorScale(n.data.event), stroke: 'white', strokeWidth: '1px', 'rx': 2 }}
                          ...
                          <line x1={n.x + nodeWidth / 2} y1={yScale(n.y)} x2={n.x + nodeWidth / 2} y2={yScale(0) + nodeWidth} style={{ stroke: 'rgb(150,150,150)', strokeWidth: '1.2px' }}></line>
                      </>
                ...
                let labels = nodes.map((n) => {
                  return <>
                    ...
                    <text
                      style={{ fontSize: ".75em", textAnchor: "middle", fill: 'rgb(90,90,90)' }}
                      x={n.x + nodeWidth / 2 - 3}
                      y={yScale(n.y) + textHeight}>
                      {" "}
                      {n.data.event}{" "}
                    </text>
                  </>
                    ...
                    return <svg width={300} height={yScale(Math.max(... nodes.map(n=>n.y)))+20} >
                                      <g style={{ "transform": "translate(50px, 0px)" }}>
