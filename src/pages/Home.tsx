...
export default function TaskCard() {
  ...
  const { tasks, conditions, actions, metrics, participants } = useContext(
                      ...
                      if (value.type == "text") {
                        return (
                          <Grid key={value.metric} item>
                            ...
                            <BarChart
                              data={value}
                              size={{ width: 200, height: 100 }}></BarChart>
                          </Grid>
                        );
                      }
                      return <></>;
            ...
            <Card className={classes.root} key={task.taskID}>
              <CardContent>
                ...
                {conditions.map((condition) => {
                  ...
                  let frequentActions = actions
                    .filter(
                      (a) => a.taskID == task.taskID && a.condition == condition
                    )
                    .splice(0, 5)
                    .map((a) => ({
                      event: a.label,
                  ...
                  let filteredMetrics = metrics.filter(
                    (m) =>
                      m.group.taskID == task.taskID &&
                      m.group.condition == condition
                  );
                  let metricValues = [
